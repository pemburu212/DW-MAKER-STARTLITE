// Popup edit style untuk layer
    ...
    const btnEsri = document.getElementById('btn-esri');
    const btnGoogle = document.getElementById('btn-google');

    btnEsri.classList.add('active');

    function setActiveButton(activeBtn) {
      [btnEsri, btnGoogle].forEach(btn => {
        btn.classList.remove('active');
      });
      activeBtn.classList.add('active');
    }

    // Event klik tombol Esri
    btnEsri.onclick = function () {
      if (!map.hasLayer(esriLayer)) {
        map.addLayer(esriLayer);
        map.removeLayer(googleLayer);
        setActiveButton(btnEsri);
      }
    };

    // Event klik tombol Google
    btnGoogle.onclick = function () {
      if (!map.hasLayer(googleLayer)) {
        map.addLayer(googleLayer);
        map.removeLayer(esriLayer);
        setActiveButton(btnGoogle);
      }
    };

    // Logika hover agar menu tidak hilang saat pindah mouse
    const wrapper = document.querySelector('.layer-hover-wrapper');
    const control = document.querySelector('.custom-layer-control');
    ...
    wrapper.addEventListener('mouseleave', () => {
      ...
      }, 200); // delay sedikit biar smooth
    });

    


    // Tampilkan zoom level di console saat zoom berubah
    map.on('zoomend', function () {
      console.log('Zoom level sekarang:', map.getZoom());
    });
    ...
    function parseKML(kmlDoc, filename) {
      ...
      for (let el of placemarkElements) {
        ...
        if (lineStringEl) {
          ...
          paths.push({ name, polyline, layerGroup: layerGroup, visible: true }); 
          ...
          if (coordPairs.length === 1) {
            ...
            const { marker, label } = addLabeledMarker(name, lat, lon, defaultIconClr);
            marker.on('click', (e) => handleClick(e.originalEvent, marker, name, lat, lon));
            placemarks.push({ name, lat, lon, marker, label, layerGroup: layerGroup, visible: true, target: targetHC });
            ...
            const polygon = L.polygon(latlngs, { color: '#ffffff', fillColor: '#aa00ff',  fillOpacity: 0.1 }).addTo(layerGroup);
  ...
  map.addLayer(layerGroup);
    ...
    let manualPathLengthTooltip = null;
    function setManualPathMode() {
      ...
      window.addEventListener('keydown',function(e){
        if (isManualPathDrawing && e.key === 'Escape'){
            map.removeLayer(manualPathLengthTooltip);
            manualPathLengthTooltip = null;
            clearManualPath()
          }
        });
      }
    ...
    function manualPathAddPoint(e) {
      ...
      for (let i = 1; i < manualPathLatLngs.length; i++) {
        const latlng1 = L.latLng(manualPathLatLngs[i - 1]);
        const latlng2 = L.latLng(manualPathLatLngs[i]);
        totalLength += latlng1.distanceTo(latlng2);
      }

      // Ambil titik terakhir
      const lastLatLng = manualPathLatLngs[manualPathLatLngs.length - 1];

      // Tampilkan atau update tooltip panjang path
      if (!manualPathLengthTooltip) {
        manualPathLengthTooltip = L.tooltip({
          permanent: true,
          direction: 'top',
          offset: [0, -10],
          className: 'manual-path-length-tooltip'
        })
        .setLatLng(lastLatLng)
        .setContent(`${totalLength.toFixed(1)} m`)
        .addTo(map);
      } else {
        manualPathLengthTooltip
          .setLatLng(lastLatLng)
          .setContent(`${totalLength.toFixed(1)} m`);
      }
    ...
    function manualPathRemoveLastPoint(e) {
      ...
      if (manualPathLatLngs.length > 0) {
        ...
        if (manualPathLatLngs.length === 0 && manualPathPolyline) {
          map.removeLayer(manualPathPolyline);
          manualPathPolyline = null;          
          if (manualPathLatLngs.length === 0 && manualPathLengthTooltip) {
            map.removeLayer(manualPathLengthTooltip);
            manualPathLengthTooltip = null;
          }
  ...
  if (e && typeof e.preventDefault === 'function') e.preventDefault();
    ...
    function handleClick(event, marker, name, lat, lon) {
      if (mode === "base") {
        // Jika Ctrl ditekan dan marker ini adalah base saat ini → unselect
        if (event.ctrlKey && basePlacemark && basePlacemark.lat === lat && basePlacemark.lon === lon) {
          resetMarkerIcon(marker,false);
          basePlacemark = null;
          return;
        }

        // Ganti base yang lama dengan yang baru
        if (basePlacemark) resetMarkerIcon(basePlacemark.marker,false);
        ...
        // ✅ Temukan placemark dari array
        const placemark = placemarks.find(p => p.lat === lat && p.lon === lon);

        // ✅ Cek apakah ditemukan dan target === true
        if (!placemark || !placemark.target) return;
        if (event.shiftKey) {
          ...
          if (index !== -1) {
            ...
            resetMarkerIcon(selected.marker, true);
    ...
    function handlePolygonClick(polygonLayer) {
      ...
      placemarks.forEach(p => {
        if (!p.visible && !p.target) return; // Skip jika marker disembunyikan dan bukan target homepass
        // console.log('Checking placemark:', p.visible, p.name);
        const point = L.latLng(p.lat, p.lon);
        if (basePlacemark && p.name === basePlacemark.name) return;

        if (p.target && leafletPointInPolygon(point, polygonLatLngs)) {
    ...
    function addLabeledMarker(name, lat, lon, color = defaultIconClr, isTarget = targetHC) {
      // Marker standar
    ...
    ensureDWFolderInFileList();
      ...
      selectedTargets.forEach(target => {
        ...
        paths.push(pathLine); // 🆕 Simpan ke array
        resetMarkerIcon(target.marker, true);
      });
      
      resetMarkerIcon(basePlacemark.marker,false);
    ...
    function resetMarkerIcon(marker, targetplcmtk) {
      const color1 = targetplcmtk ? 'green' : 'blue';
      console.log('Resetting marker icon to color:', color1);
      marker.setIcon(L.icon({
        iconUrl: `https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-${color1}.png`,
        shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/0.7.7/images/marker-shadow.png',
        iconSize: [25, 41],
        iconAnchor: [12, 41]
      }));
    ...
    function UnloadAll() {
      ...
      if (dwList) dwList.innerHTML = "";
          ...
          selectedTargets.forEach(t => resetMarkerIcon(t.marker), true);
          if (basePlacemark) resetMarkerIcon(basePlacemark.marker,false);
  ...
  if (dwList) dwList.innerHTML = "";
        ...
        placemarks.forEach(p => resetMarkerIcon(p.marker , p.target));
